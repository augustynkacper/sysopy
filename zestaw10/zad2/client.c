...
void handler(int){
    sendto(socket_id, "STOP", MAX_MESSAGE_LENGTH, 0, NULL, sizeof(struct sockaddr_in));
    printf("left\n");
    exit(1);
}
...
void send_message(char* msg, int fd){
    ...
    sendto(fd,  msg, MAX_MESSAGE_LENGTH, 0, NULL, sizeof(struct sockaddr_in));
}
    ...
    while(1){
        ...
        for (int i=0; i<events_occured; i++) {
            if (events[i].data.fd == STDIN_FILENO){
                ...
                char buffer[256];
                long n = recvfrom(socket_id, buffer, sizeof buffer, 0, NULL, NULL);
                buffer[n] = 0;
                //puts(buffer);
...
int local_connection(char * path){
    ...
    strncpy(bind_addr.sun_path, path, sizeof bind_addr.sun_path);
    int local_socket = socket(AF_UNIX, SOCK_DGRAM, 0);

    if ( bind(local_socket, (struct sockaddr*) &bind_addr, sizeof bind_addr) <=0 ){
        printf("bind error %s\n", strerror(errno));
    }
